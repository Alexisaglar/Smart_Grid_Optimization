...
class LSTMForecastModel(nn.Module):
    ...
    def __init__(self, input_size=1, hidden_layer_size=128, num_layers=2, output_size=1, dropout=0.1):
        super().__init__()
        self.lstm = nn.LSTM(
...
def evaluate_full_year(
    ...
    # --- FIX APPLIED HERE ---
    # Create a validation-specific TimeSeriesDataSet to get the correct dataloader
    validation_tft_dataset = TimeSeriesDataSet.from_dataset(
        training_dataset, validation_df, stop_randomization=True
    )
    val_dataloader = validation_tft_dataset.to_dataloader(
        train=False, batch_size=BATCH_SIZE * 10, num_workers=0
    )
    # --- END OF FIX ---

    tft_raw_preds = tft_model.predict(val_dataloader, mode="raw", return_index=True)
...
def plot_results(results_df, maes, predicting, simulation_hours):
    ...
    ax.plot(plot_df.index, plot_df['day_ahead_pred'], label=f'TFT', color='#D55E00', linestyle='--', linewidth=2, zorder=4)
    ax.plot(plot_df.index, plot_df['rolling_pred_p50'], label=f'TFT Rolling Horizon', color='#0072B2', linewidth=2, zorder=6)
    ax.fill_between(plot_df.index, plot_df['rolling_pred_p10'], plot_df['rolling_pred_p90'], color='#56B4E9', alpha=0.3, label='TFT Rolling Horizon 10th-90th Percentile')
