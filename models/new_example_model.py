...
def create_datasets(
    ...
    # Remove target and time_idx from inputs
...
def plot_strategy_comparison(results_df: pd.DataFrame, mae_day_ahead: float, mae_rolling: float, predicting: str):
    """
    Creates a high-quality, publication-ready figure comparing the two forecasting strategies.
    """
    logger.info("Generating publication-quality figure for strategy comparison...")
...
def run_comparison_simulation(
    ...
    logger.info(f"--- Starting Forecast Comparison Simulation for {predicting} ---")
    ...
    for t in range(simulation_hours):
        ...
        if t % 24 == 0:
            ...
            day_ahead_full_forecast = model.predict(prediction_input_df, mode="quantiles")

        day_ahead_pred = day_ahead_full_forecast[0, t % 24, 3].item() # Index 3 is the 50th percentile (median)
        ...
        rolling_full_forecast = model.predict(rolling_input_df, mode="quantiles")
        
        rolling_p10 = rolling_full_forecast[0, 0, 0].item()
        rolling_p50 = rolling_full_forecast[0, 0, 3].item()
        rolling_p90 = rolling_full_forecast[0, 0, 6].item()

        # --- 3. Get Actual Value ---
        actual = full_df.loc[full_df.time_idx == current_time_idx, predicting].iloc[0]
        
        # Store results, converting temp back to Celsius if needed
        if predicting == "GHI":
            results.append({
                'timestamp': current_datetime, 'actual': actual, 'day_ahead_pred': day_ahead_pred,
                'rolling_pred_p10': rolling_p10, 'rolling_pred_p50': rolling_p50, 'rolling_pred_p90': rolling_p90,
            })
        else: # Assuming temperature
            results.append({
                'timestamp': current_datetime, 'actual': actual - 273.15, 'day_ahead_pred': day_ahead_pred - 273.15,
                'rolling_pred_p10': rolling_p10 - 273.15, 'rolling_pred_p50': rolling_p50 - 273.15, 'rolling_pred_p90': rolling_p90 - 273.15,
            })
    ...
    mae_rolling = (results_df['actual'] - results_df['rolling_pred_p50']).abs().mean()

    logger.info(f"--- Simulation Results for {predicting} ---")
    ...
    plot_strategy_comparison(results_df, mae_day_ahead, mae_rolling, predicting)
...
def main():
    ...
    if args.train:
        logger.info(f"--- Starting Model Training for {args.predicting} ---")
