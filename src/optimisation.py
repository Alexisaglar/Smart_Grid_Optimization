...
def solve_VPP_optimisation(
    ...
    ev_present_matrix: np.array,
    pv_power_forecasts: np.array
) -> np.array:
    """
    Solves the VPP optimisation problem for EV and BESS scheduling.
    Args:
    ...
    num_lots = config.TOTAL_NUM_EVS
    num_evs = len(ev_data)
    T = config.NUM_TIME_STEPS
    time_steps = range(T)
    dt = config.TIME_STEP_DURATION_H

    # Decision variables (positive for charging, negative for discharging)
    # Bess variables
    ...
    # Total power exchanged trough Smart Grid scheduling
    total_sg_power_per_lot = cp.Variable((num_lots, T), name="total_sg_power")
    ...
    # EV Constraints
    constraints += [ev_soc[:, 0] == ev_data['soc_at_arrival'].values] # Initial SOC
    ...
    for lot in range(num_lots):
        evs_in_lot = ev_data[ev_data['ev_id'] == lot].index
        if len(evs_in_lot) > 0:
             constraints += [total_sg_power_per_lot[lot,:] == cp.sum(ev_power[evs_in_lot, :], axis=0) + bess_power[lot,:] - pv_power[lot,:]]
        else:
             constraints += [total_sg_power_per_lot[lot,:] == bess_power[lot,:] - pv_power[lot,:]]

    total_grid_exchange_power = cp.sum(total_sg_power_per_lot, axis=0)
