...
def calculate_full_schedule(
    ...
    enable_dr: bool = True,
    dr_participant_list: list = None,
    consumer_coeffs: dict = None,
    participant_limits: dict = None
) -> pd.DataFrame:
    # --- Data Mapping ---
    ...
    model = pyo.ConcreteModel("Full Coordinated Dispatch with Pre-defined DR")
    ...
    # Create a set for the known DR participants
    if enable_dr and dr_participant_list:
        model.DR_PARTICIPANT_BUS_IDs = pyo.Set(initialize=dr_participant_list)

    if enable_dr and participant_limits:
        model.participant_limit = pyo.Param(model.DR_PARTICIPANT_BUS_IDs, initialize=participant_limits)

    else:
        model.DR_PARTICIPANT_BUS_IDs = pyo.Set(initialize=[])

    if enable_dr and consumer_coeffs:
        model.consumer_phi = pyo.Param(model.DR_PARTICIPANT_BUS_IDs, initialize=consumer_coeffs)
    ...
    if enable_dr:
        # Curtailment variable is now indexed by the specific participant list
        model.p_curtailment = pyo.Var(model.DR_PARTICIPANT_BUS_IDs, model.T, domain=pyo.NonNegativeReals)
        model.c_incentive = pyo.Var(model.T, domain=pyo.NonNegativeReals)
    else:
        # Default to 0 if DR is disabled
    ...
    def full_objective_rule(model):
        ...
        dr_profit = 0
        if enable_dr and model.DR_PARTICIPANT_BUS_IDs:
            # Sum over only the known participants
            dr_profit = sum(
                (grid_price[t] - model.c_incentive[t]) * model.p_curtailment[i, t]
                for i in model.DR_PARTICIPANT_BUS_IDs for t in model.T
            )
    ...
    def active_power_balance_rule(model, t, i):
        ...
        # Check if the bus 'i' is in the participant set
        curtailed_load_p = model.p_curtailment[i, t] if enable_dr and i in model.DR_PARTICIPANT_BUS_IDs else 0
    ...
    # --- DR CONSTRAINTS (now indexed by the participant set) ---
    if enable_dr and model.DR_PARTICIPANT_BUS_IDs:
        def max_curtailment_rule(model, i, t):
            base_load_p = base_loads_p.get(i, 0) * (load_forecasts / load_forecasts.max())[t]
            return model.p_curtailment[i, t] <= 0.40 * base_load_p
        model.max_curtailment_constraint = pyo.Constraint(model.DR_PARTICIPANT_BUS_IDs, model.T, rule=max_curtailment_rule)
        ...
        def total_energy_curtailment_rule(model, i):
            ...
            # --- MODIFIED: Use the specific limit for bus 'i' ---
            limit = model.participant_limit[i]
            return total_curtailed_energy <= limit * total_base_energy
        model.total_energy_curtailment_constraint = pyo.Constraint(model.DR_PARTICIPANT_BUS_IDs, rule=total_energy_curtailment_rule) 

        def consumer_benefit_rule(model, i):
            # --- MODIFIED: Use the specific phi for bus 'i' ---
            phi = model.consumer_phi[i]
        ...
        model.consumer_benefit_constraint = pyo.Constraint(model.DR_PARTICIPANT_BUS_IDs, rule=consumer_benefit_rule)

        def total_incentive_budget_rule(model):
            total_incentive_paid = sum(model.c_incentive[t] * model.p_curtailment[i, t] for i in model.DR_PARTICIPANT_BUS_IDs for t in model.T)
    ...
    # --- Power Flow and Voltage Constraints ---
    ...
    if (results.solver.status == pyo.SolverStatus.ok) and (results.solver.termination_condition == pyo.TerminationCondition.optimal):
        ...
        if enable_dr and model.DR_PARTICIPANT_BUS_IDs:
            # --- MODIFIED: Extract curtailment for each participant individually ---
            res['c_incentive'] = [pyo.value(model.c_incentive[t]) for t in model.T]
            
            total_curtailment = np.zeros(forecast_range)
            for i in model.DR_PARTICIPANT_BUS_IDs:
                participant_curtailment = [pyo.value(model.p_curtailment[i, t]) for t in model.T]
                res[f'curtailment_bus_{i}'] = participant_curtailment
                total_curtailment += np.array(participant_curtailment)

            res['curtailed_load'] = total_curtailment # Keep the total for other calculations
            res['dr_participants'] = str(dr_participant_list) 
...
def calculate_kpis(schedule_df, bess_objects, grid_price, dr_participants_list):
    """
    Calculates a comprehensive set of KPIs, now including per-participant incentives.
    Returns two dictionaries: one for overall KPIs, one for participant-specific incentives.
    """
    kpis = {}
    participant_incentives = {}

    # --- Standard Cost Components (Unchanged) ---
    ...
    # --- Detailed DR Program Metrics ---
    if 'curtailed_load' in schedule_df.columns and 'c_incentive' in schedule_df.columns:
        # --- NEW: Calculate incentive for each participant ---
        for bus_id in dr_participants_list:
            curtailment_col = f'curtailment_bus_{bus_id}'
            if curtailment_col in schedule_df.columns:
                incentive = (schedule_df[curtailment_col] * schedule_df['c_incentive']).sum()
                participant_incentives[f'incentive_bus_{bus_id}'] = incentive

        incentive_paid = sum(participant_incentives.values())
        grid_savings = (schedule_df['curtailed_load'] * grid_price).sum()
        
        kpis['dr_incentive_paid'] = incentive_paid
        kpis['dr_profit'] = grid_savings - incentive_paid
        kpis['total_energy_curtailed_mwh'] = schedule_df['curtailed_load'].sum()
        kpis['num_dr_participants'] = len(dr_participants_list)
        
        if kpis['total_energy_curtailed_mwh'] > 1e-6:
            kpis['avg_incentive_rate_eur_per_mwh'] = incentive_paid / kpis['total_energy_curtailed_mwh']
        else:
            kpis['avg_incentive_rate_eur_per_mwh'] = 0
    else:
        # Set default DR values to 0 if DR was not enabled
        kpis.update({
            'dr_incentive_paid': 0, 'dr_profit': 0, 'total_energy_curtailed_mwh': 0,
            'num_dr_participants': 0, 'avg_incentive_rate_eur_per_mwh': 0
        })

    # --- Total Cost & Other Metrics (Unchanged) ---
    kpis['total_expenditure'] = kpis['grid_cost'] + kpis['degradation_cost'] + kpis.get('dr_incentive_paid', 0)
    ...
    return kpis, participant_incentives
...
def plot_cost_savings_comparison(kpi_df):
    ...
    cost_data = kpi_df.pivot(index='Forecast Model', columns='Scenario', values='total_expenditure')
...
def create_participant_analysis_table(schedule_storage, dr_participants, net, total_load_forecast):
    """
    Analyzes DR participation for the 'Actual' and 'TFT Rolling Horizon' forecasts.

    Calculates incentive and curtailment percentage for each participant.
    """
    analysis_data = []
    base_loads_p = net.load.set_index('bus')['p_mw']
    load_scaling_factor = total_load_forecast / total_load_forecast.max()

    for scenario in ['Advanced (Emerging PV)', 'Baseline (All Silicon)']:
        try:
            # Get the schedules for the two key forecast models
            df_actual = schedule_storage[(scenario, 'Actual')]
            df_tft = schedule_storage[(scenario, 'TFT Rolling Horizon')]
        except KeyError:
            # Skip if a scenario failed to solve
            continue

        for bus_id in dr_participants:
            # Calculate the total original demand for this bus over 24h
            base_load = base_loads_p.get(bus_id, 0)
            total_demand_mwh = (base_load * load_scaling_factor).sum()

            # --- Process 'Actual' Forecast Data ---
            curtail_col = f'curtailment_bus_{bus_id}'
            total_curtailed_actual = df_actual[curtail_col].sum()
            incentive_actual = (df_actual[curtail_col] * df_actual['c_incentive']).sum()
            percent_curtailed_actual = (total_curtailed_actual / total_demand_mwh) * 100 if total_demand_mwh > 0 else 0

            # --- Process 'TFT Rolling Horizon' Forecast Data ---
            total_curtailed_tft = df_tft[curtail_col].sum()
            incentive_tft = (df_tft[curtail_col] * df_tft['c_incentive']).sum()
            percent_curtailed_tft = (total_curtailed_tft / total_demand_mwh) * 100 if total_demand_mwh > 0 else 0

            analysis_data.append({
                'Scenario': scenario,
                'Bus': bus_id,
                'Incentive Actual (£)': incentive_actual,
                'Incentive TFT RH (£)': incentive_tft,
                'Curtailment Actual (%)': percent_curtailed_actual,
                'Curtailment TFT RH (%)': percent_curtailed_tft
            })
...
def plot_dr_stacked_bar(schedule_df, dr_participants, net, total_load_forecast, title_suffix="", filename="dr_participation_stacked.png"):
    """
    Creates a stacked bar chart showing the consumed and curtailed power
    for each participant in the DR program, similar to the provided example.
    """
    ...
    for bus_id in dr_participants:
        base_load = base_loads_p.get(bus_id, 0)
        original_demand_mw = base_load * load_scaling_factor
        curtailed_mw = schedule_df[f'curtailment_bus_{bus_id}']
        consumed_mw = original_demand_mw - curtailed_mw
        consumed_data[f'Bus {bus_id}'] = consumed_mw * 1000 # Convert to kW
        curtailed_data[f'Bus {bus_id}'] = curtailed_mw * 1000 # Convert to kW

    consumed_df = pd.DataFrame(consumed_data)
    curtailed_df = pd.DataFrame(curtailed_data)
    ...
    # --- 3. Plot Stacked Bars ---
    bottom_consumed = np.zeros(len(hours))
    for i, (col, color) in enumerate(zip(consumed_df.columns, colors.colors)):
        ax.bar(hours, consumed_df[col], bottom=bottom_consumed, width=bar_width,
               label=col, color=color, edgecolor='black', linewidth=0.5)
        bottom_consumed += consumed_df[col]

    bottom_curtailed = np.zeros(len(hours))
    for i, (col, color) in enumerate(zip(curtailed_df.columns, colors.colors)):
        ax.bar(hours, -curtailed_df[col], bottom=bottom_curtailed, width=bar_width,
               color=color, edgecolor='black', linewidth=0.5)
        bottom_curtailed -= curtailed_df[col]

    # --- 4. Formatting and Annotations ---
    ax.axhline(0, color='black', linewidth=1.5)
    # ax.set_title(f'DR Participant Dispatch Summary: {title_suffix}') # MODIFIED: Re-enabled title
    ax.set_xlabel('Time (Hour)')
    ax.set_ylabel('Power (kW)')
    
    # MODIFIED: Create a secondary y-axis on the right for the annotations
    ax2 = ax.twinx()
    ax2.set_ylim(ax.get_ylim()) # Match the limits of the left axis
    ...
    ax.text(1.03, 0.25, 'Load Curtailed', transform=ax.transAxes, 
            ha='center', va='center', rotation=90, fontsize=26)
    ...
    # MODIFIED: Use subplots_adjust to make room for the legend and labels
...
if __name__ == "__main__":
    DR_PARTICIPANTS = [5, 12, 17, 21, 25, 26, 30, 32]
    CONSUMER_DISCOMFORT_COEFFS = {
        5: 0.8,   # Commercial bus, less tolerant
        12: 0.9,   # Residential bus, tolerant
        17: 0.4,  # Industrial bus, more tolerant
        21: 0.9,  # Residential bus, very sensitive
        25: 0.3,  # Residential bus, very sensitive
        26: 0.9,  # Residential bus, very sensitive
        30: 0.6,  # Residential bus, medium tolerance
        32: 0.3,  # Residential bus, medium tolerance
    }

    PARTICIPANT_DAILY_LIMITS = {
        5: 0.25,  # Commercial bus: 25%
        12: 0.40, # Tolerant residential: 40%
        17: 0.50, # Industrial bus: 50%
        21: 0.15, # Sensitive residential: 15%
        25: 0.30, 
        26: 0.35,
        30: 0.35,
        32: 0.30
    ...
    # --- Set the number of DR participants for the simulation ---
    # NUM_DR_PARTICIPANTS_TO_SELECT = 3
    ...
    for scenario_name, param_list in scenarios.items():
        ...
        for model_name, col_name in forecast_models.items():
            ...
            optimal_df, _ = calculate_full_schedule(
                ...
                dr_participant_list=DR_PARTICIPANTS,
                consumer_coeffs=CONSUMER_DISCOMFORT_COEFFS,
                participant_limits=PARTICIPANT_DAILY_LIMITS
            )

            if optimal_df is not None:
                # --- MODIFIED: Get the list of participants and call the new KPI function ---
                # The 'dr_participants' key is added during results extraction in Calculate_full_schedule
                participants = eval(optimal_df['dr_participants'].iloc[0]) if 'dr_participants' in optimal_df.columns else []

                kpis, participant_incentives = calculate_kpis(
                    optimal_df, all_bess, grid_price_data, DR_PARTICIPANTS
                )

                # kpis = calculate_kpis(optimal_df, all_bess, grid_price_data, network.load.p_mw.sum(), participants)
                kpis['Scenario'] = scenario_name
                kpis['Forecast Model'] = model_name
                all_kpis.append(kpis)

                
                # --- NEW: Store the per-participant results ---
                participant_incentives['Scenario'] = scenario_name
                participant_incentives['Forecast Model'] = model_name
                per_participant_results.append(participant_incentives)
    ...
    if all_kpis:
        ...
        # Set pandas display options for clean, readable tables in your terminal
        pd.set_option('display.width', 200)
        pd.set_option('display.max_columns', 10)
        pd.set_option('display.float_format', '{:,.2f}'.format)

        # --- TABLE 1: DETAILED COST & PROFIT BREAKDOWN ---
        # Define the specific columns we want for the first table
        cost_cols = [
            'Scenario', 
            'Forecast Model', 
            'total_expenditure', 
        ...
        # Select, sort, and print the first table
        cost_table_df = kpi_results_df[cost_cols].sort_values(by=['Scenario', 'total_expenditure'])
        
        print("\n" + "="*120)
        print("TABLE 1: DETAILED COST & PROFIT BREAKDOWN (£)")
        print("="*120)
        print(cost_table_df.to_string(index=False))
        print("="*120 + "\n")

        # --- TABLE 2: DEMAND RESPONSE & SYSTEM SUMMARY ---
        # Define the specific columns we want for the second table
        dr_cols = [
            'Scenario', 
            'Forecast Model', 
            'num_dr_participants', 
            'total_energy_curtailed_mwh',
            'avg_incentive_rate_eur_per_mwh', 
            'self_sufficiency_rate_pct'
        ]
        
        # Select, sort, and print the second table
        dr_table_df = kpi_results_df[dr_cols].sort_values(by=['Scenario', 'Forecast Model'])

        print("\n" + "="*120)
        print("TABLE 2: DEMAND RESPONSE & SYSTEM SUMMARY")
        print("="*120)
        print(dr_table_df.to_string(index=False))
        print("="*120 + "\n")

        # --- NEW: Code to generate and print Table 3 ---
        participant_df = pd.DataFrame(per_participant_results)
        
        # Reorder columns for better presentation
        cols = ['Scenario', 'Forecast Model'] + sorted([col for col in participant_df.columns if col.startswith('incentive_bus_')])
        participant_df = participant_df[cols]
        
        print("\n" + "="*120)
        print("TABLE 3: INCENTIVE PER PARTICIPANT (£)")
        print("="*120)
        print(participant_df.to_string(index=False))
        print("="*120 + "\n")
        # --- Your existing plotting calls can remain here ---
        print("\n--- Generating Final Comparison Plots ---")
        if schedule_storage:
            participant_analysis_df = create_participant_analysis_table(
                schedule_storage,
                DR_PARTICIPANTS,
                network,
                total_load_forecast
            ...
            print("\n" + "="*120)
            print("TABLE 3: PARTICIPANT-LEVEL DR ANALYSIS")
            print("="*120)
            print(participant_analysis_df.to_string(index=False))
            print("="*120 + "\n")

        plot_cost_savings_comparison(kpi_results_df)

        # Example of plotting the best-case dispatch
        if ('Advanced (Emerging PV)', 'TFT Rolling Horizon') in schedule_storage:
            best_df = schedule_storage[('Advanced (Emerging PV)', 'TFT Rolling Horizon')]
            plot_thesis_dispatch_bars(
                best_df,
                title_suffix="Advanced Scenario (TFT Rolling Horizon)",
                filename="dispatch_final_plot.png"
            )

        plot_dr_stacked_bar(
            schedule_df=best_df,
            dr_participants=DR_PARTICIPANTS,
            net=network,
            total_load_forecast=total_load_forecast,
            title_suffix="Advanced Scenario (TFT Rolling)",
            filename="dr_participation_stacked.png"
        )
    else:
